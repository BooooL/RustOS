...
pub mod imp;
#[cfg(all(not(target_env = "msvc"), not(feature = "rustos")))] #[path = "gcc.rs"] #[doc(hidden)]
pub mod imp;
...
pub mod imp {
    ...
    pub fn panic(cause: Box<Any + Send + 'static>) -> ! {
        loop {}
    }
    
    pub fn cleanup(ptr: *mut c_void) -> Box<Any + Send + 'static> {
        unimplemented!();
    }
